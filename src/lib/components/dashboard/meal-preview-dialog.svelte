<script lang="ts">
	...
	const openDeleteConfirm = () => {
		if (!meal || !onmealdelete || deleteConfirmOpen) return;
	...
	$effect(() => {
		...
		const updatePreviewMetrics = () => {
			...
			heroHeight = element.offsetHeight;
		};
	...
	<Dialog.Portal>
		...
		>
					...
					bindings: [
						{
							...
							when: () => open && Boolean(meal && onmealdelete && !deleteConfirmOpen)
						}
					]
				}}
				class="contents"
			>
				{#if meal}
					<Sheet.Frame
						leadIn={previewLeadIn}
						closeLabel="Close meal preview"
						onclose={() => (open = false)}
					>
