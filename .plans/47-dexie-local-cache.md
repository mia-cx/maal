...
- 2026-06-19: Fixed existing app layout raw href lint violations while touching the layout by adding `$app/paths` `resolve()` for subscribe/export links.
- Validation: `pnpm prettier --check .plans/47-dexie-local-cache.md src/lib/client-db src/lib/stores/route-data-cache.ts 'src/routes/(app)/plan/+page.svelte' 'src/routes/(app)/menu/+page.svelte' 'src/routes/(app)/+layout.svelte'` — pass.
