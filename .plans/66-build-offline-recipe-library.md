...
- [x] Recipe headers and all seven sidecar families preserve every implemented prototype field.
- [x] Create, edit, search, delete, restore, and permanent delete work offline through commands and live queries.
- [x] Deleted content remains recoverable for 30 days and minimal purge tombstones remain for one year.
- [x] Imported candidates retain provenance and URL media, and only become recipes through the normal local command after confirmation.
- [x] Existing meals are not removed when a source recipe is permanently deleted.
- [x] The approved prototype recipe UI is ported without redesign.
...
- Caveat: Vite reports the lazy menu route chunk at 539 kB minified (164 kB gzip).
- Integration validation: `pnpm validate` passed with 108 unit/browser tests and 5 Playwright tests. The manifest-based initial `/` graph is 42,196 / 256,000 gzip bytes; lazy household and menu route chunks stay outside that budget.
