...
- [x] Validation commands from the issue pass or residual risks are documented.
...
- [x] Run final HTTP/module validation and file the PR.
...
- Initial `pnpm check` failed because `worker-configuration.d.ts` was missing in the worktree; ran `pnpm wrangler types` to generate ignored local Cloudflare types.
- `pnpm check && pnpm test:unit -- --run src/lib/server/http && pnpm architecture:check` passed.
