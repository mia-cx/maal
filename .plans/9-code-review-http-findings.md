...
## Acceptance criteria
- [ ] JSON request helpers reject arrays and non-plain object bodies.
- [ ] Routes can require authenticated household context without implicitly enforcing billing access.
- [ ] Billing-gated routes opt in to entitlement checks through a dedicated helper.
- [ ] Known domain error translation uses typed identifiers or explicit key lookups and remains integrated with routes.
...
## TODOs
- [ ] Tighten `readJsonObject` plain-object validation and add request regression tests.
- [ ] Split app context into base and billing-gated helpers, then update routes to opt in intentionally.
- [ ] Replace brittle known-error mapping with typed domain error codes and update route/service call sites.
