...
- [x] Run focused and repo validation, then file PR.
...
- Validation: `pnpm check` passed after generating ignored `worker-configuration.d.ts` with `pnpm gen`.
- Validation: `pnpm test:unit -- --run src/lib/server/services` passed.
- Validation: `pnpm architecture:check` passed.
- `pnpm lint` still reports pre-existing Prettier drift across hundreds of files; changed files were formatted with Prettier directly.
